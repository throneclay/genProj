...
void cTemplate::genFunction(string &ffunc,const string funcname)
{
	...
	// add a random generater
	string ra="void genRandomf(float* r, int length, float min, float max){\n\n";
	ra +="\tsrand(time(NULL));\n\tfor(int i=0;i<length;i++)\n\t\tr[i]=rand()/RAND_MAX*(max-min)+min;\n}\n";
	ra +="void genRandomi(int* r, int length, float min, float max){\n\n";
	ra +="\tsrand(time(NULL));\n\tfor(int i=0;i<length;i++)\n\t\tr[i]=(float)rand()*(max-min)/RAND_MAX+min;\n}\n";
	ffunc+=ra;
}
